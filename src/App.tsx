                  ...
                  roomState.session.boardSource === 'generated' ? (
                    isHost ? (
                      <LobbyPanel
                        ...
                        startLabel={`Rumble ${currentRound.gridSize ** 2} cubes & start`}
                      />
                    ) : <WaitingPanel icon={<Gamepad2 />} title="Waiting for the host to rumble the cubes" copy="Keep this screen open. Everyone gets the same rumbled board after a synchronized three-second countdown." />
                  ) : isHost ? (
                    <>
  ...
  return (
    <section className="single-form-wrap create-wrap">
      ...
      <div className="create-card">
        ...
        {props.createMode === 'scribbage' && <div className="setup-block"><span className="setup-number">03</span><div><h2>Choose the board</h2><div className="board-source-picker"><button className={props.boardSource === 'generated' ? 'is-selected' : ''} type="button" onClick={() => props.setBoardSource('generated')}><Gamepad2 /><span><strong>Rumbled cubes</strong><small>Shake the fixed physical-style cube set online</small></span>{props.boardSource === 'generated' && <Check />}</button><button className={props.boardSource === 'physical' ? 'is-selected' : ''} type="button" onClick={() => props.setBoardSource('physical')}><Camera /><span><strong>Physical board</strong><small>Scan your real tile setup</small></span>{props.boardSource === 'physical' && <Check />}</button></div></div></div>}
        {props.createMode !== 'scrabble' && <div className="setup-block"><span className="setup-number">{props.createMode === 'scribbage' ? '04' : '03'}</span><div className="setup-grid"><label><span>Board size</span><select value={props.gridSize} onChange={(event) => props.setGridSize(Number(event.target.value) as 4 | 5)}><option value="4">4 × 4{props.createMode === 'scribbage' ? ' · compact' : ''}</option><option value="5">5 × 5{props.createMode === 'scribbage' ? ' · physical Word Factory' : ''}</option></select></label><label><span>Round timer</span><select value={props.timerSeconds} onChange={(event) => props.setTimerSeconds(Number(event.target.value))}>{!(props.createMode === 'scribbage' && props.boardSource === 'generated') && <option value="0">No timer</option>}<option value="120">2 minutes</option><option value="180">3 minutes</option><option value="300">5 minutes</option></select></label></div></div>}
        <div className="create-action"><p><Users size={16} /> Up to {props.playerLimit} players · {modeLabel(props.createMode)}{props.createMode === 'scribbage' ? ` · ${props.boardSource === 'generated' ? 'Rumbled cubes' : 'Physical board'}` : ''}</p><button className="primary-button" type="button" onClick={props.onCreate} disabled={props.busy}>{props.busy ? 'Creating…' : 'Create room'} <ArrowRight size={18} /></button></div>
      </div>
    </section>
  )
}
